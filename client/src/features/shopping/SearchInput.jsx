...
export default function SearchInput({ compact = false, disabled, onSubmit }) {
  ...
  async function handleSubmit(event) {
    ...
    const selections = compact
      ? []
      : selectionSummary({ category: 'pants', maxPrice, size });
  ...
  return (
    ...
    >
      ...
      >
        ...
        {hasQuery && !compact && (
          <div className={`filter-reveal order-2 grid grid-cols-2 border-t border-stone-200 sm:order-3 sm:basis-full ${compact ? 'gap-2 pt-2' : 'gap-3 pt-3'}`}>
      ...
      {!hasQuery && !compact && (
        <p className="mt-3 px-2 text-sm text-stone-500">문장을 입력하면 가격·사이즈 조건을 더할 수 있어요.</p>
      )}
    </form>
  );
}
