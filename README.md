...
실제 상품 정보와 구매후기를 근거로 조건에 맞는 옷을 찾아주는 여성 의류 탐색 Agent입니다. 자연어로 조건을 말하면 실제로 수집한 상품 중 최대 3개를 찾아주고, 왜 조건에 맞는지와 판매 사이즈, 검증된 실측표, 후기 신호, 구매 전 확인사항까지 한 화면에서 비교할 수 있습니다.

## Live Demo

[levit-problem-solver.onrender.com](https://levit-problem-solver.onrender.com)

Render 무료 인스턴스는 비활성 상태에서 다시 시작할 때 첫 접속이나 검색에 1분가량 또는 그 이상의 지연이 발생할 수 있습니다.

## Project Status

핵심 MVP 구현과 Render 배포까지 마쳤습니다. 현재 10개 쇼핑몰, 5개 category의 실제 상품 520개와 후기 3,798개를 검색할 수 있고, 전체 자동 테스트 118개와 확장 catalog 대상 자연스러움 평가 20개를 통과한 상태입니다.
...
옷을 살 때는 가격이나 색상, 사이즈로 후보를 좁힌 뒤에도 사진과 실물의 차이, 실제 착용 사이즈와 핏, 소재 품질을 여러 후기에서 직접 확인해야 합니다. 이 프로젝트의 목표는 검색 결과를 늘리는 것이 아니라, 조건에 맞는 후보를 줄이고 판단 근거와 불확실성을 함께 보여주는 것입니다.
...
이 결과를 반영해 조건에 맞는 상품을 먼저 찾고, 실제 구매후기는 추천 결과를 확인하는 근거로 활용했습니다. 사이즈는 확정적으로 추천하는 대신 판매 옵션과 실측 사이즈표, 후기에서 확인된 신호, 그리고 정보가 부족한 부분을 함께 보여줍니다.
...
- 후기의 상반된 의견: 하나의 축에서 의견이 명시적으로 갈리면 좋은 쪽으로 합치지 않고 `mixed`로 집계했습니다.
- 색상·사이즈 missing data: 값이 없다고 불일치로 단정하지 않고 unknown candidate로 남겨 recall을 지켰습니다.
