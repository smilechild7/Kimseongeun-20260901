...
`sizeGuideText`는 nullable이다. 쇼핑몰별 schema를 영구 정규화하는 데 과투자하지 않고, 크롤러가 실제로 읽은 원문만 저장한다. 단, deterministic strict parser로 표의 header·판매 사이즈별 치수 개수를 검증할 수 있는 원문만 허용한다. Frontend는 검증된 원문을 접근 가능한 HTML table로 변환하며, parsing에 실패하거나 사이즈표가 이미지로만 제공되면 `null`로 두고 표시하지 않는다. LLM으로 치수를 추론하거나 보완하지 않는다.
...
사이즈표는 strict parser 검증에 성공한 경우에만 표로 표시하고, 없거나 검증에 실패하면 해당 사이즈표 영역을 숨긴다. 판매 사이즈처럼 핵심 factual field가 없으면 `판단할 정보가 부족해요`라고 표시한다.
